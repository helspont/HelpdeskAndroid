...
public class MainActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		/*
		 * mydata = getValue(); item = new String [mydata.size()]; for (int i=0;
		 * i < mydata.size(); i++){ item [i] = mydata.get(i).getName(); }
		 * 
		 * for (int i=0; i < mydata.size(); i++){ System.out.println(item [i]);
		 * }
		 * 
		 * list.setAdapter(new
